 ...
 * {@link QualifiedSqlArrayTypeFactory} for the given type information.  Note
 ...
 */
@Beta
public class QualifiedSqlArrayArgumentFactory implements QualifiedArgumentFactory {
    @Override
    public Optional<Argument> build(QualifiedType qualifiedType, Object value, ConfigRegistry config) {
        ...
        Function<QualifiedType, Optional<SqlArrayType<?>>> lookup =
                qualifiedElementType -> config.get(SqlArrayTypes.class).findFor(qualifiedElementType);

        if (erasedType.isArray()) {
            QualifiedType elementType = QualifiedType.of(erasedType.getComponentType(), qualifiers);
        ...
        return GenericTypes.findGenericParameter(type, Collection.class)
                .map(elementType -> QualifiedType.of(elementType, qualifiers))
                .flatMap(lookup)
                .map(arrayType -> new SqlArrayArgument<>(arrayType, value));
    }
}
